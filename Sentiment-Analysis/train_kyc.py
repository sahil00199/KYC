...
from transformers.modeling_bert import BertForSequenceClassificationDomain as BertForSequenceClassification
...
domainVects, domainNameToUidMapping = None, None
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, index) in enumerate(randomIndices):
            ...
            if verbosity >= 2 and i%1000==10:
                ...
                print("label=",label)
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label, domain = domainName))
        return examples
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
                InputFeatures(
                        ...
                        label_id=label_id,
                        domain = domainNameToUidMapping[example.domain]))
...
def evaluate(model, dataloader, device, global_step = None, return_preds = False):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc="Iteration")):
        input_ids, input_mask, segment_ids, label_ids, domainIds = batch
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        label_ids = label_ids.to(device)
        domain_vectors = torch.stack([domainVects[domain_uid.item()] for domain_uid in domainIds]).to(device)
                
        with torch.no_grad():
            tmp_eval_loss, logits = model(input_ids, segment_ids, input_mask, label_ids, domain_vectors)
...
def initDomainVect(args):
    global domainVects, domainNameToUidMapping
    domainVects = pickle.load(open(os.path.join(args.data_dir, 'dist_vecs.pkl'), 'rb'))
    domainNameToUidMapping = {k: i for i, k in enumerate(domainVects.keys())}
    domainVects = {domainNameToUidMapping[k]: torch.from_numpy(v).type(torch.FloatTensor) for k, v in domainVects.items()}
...
def main():
    def get_data_loader(examples, batch_size):
        ...
        all_domain_ids = torch.tensor([f.domain for f in features], dtype = torch.long)

        data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_domain_ids)
    ...
    bert_config.hidden_domain_vect_size = 128
    bert_config.domain_vect_size = len(domainVects[list(domainVects.keys())[0]])
    ...
    if args.do_train:
        ...
        all_domain_ids = torch.tensor([f.domain for f in train_features], dtype = torch.long)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, _ = model(input_ids, segment_ids, input_mask, label_ids, domain_vectors)
                if n_gpu > 1:
